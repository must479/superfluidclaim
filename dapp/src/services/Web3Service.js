  ...
  function () {
    angular
      ...
      .service("Web3Service", function ($window, Utils, $uibModal, Connection, Config, $http) {
         ...
         */
        factory.enableMetamask = async function (callback) {
          try {
         ...
         **/
        factory.reloadWeb3Provider = async function (resolve, reject) {
          factory.accounts = [];
          factory.coinbase = null;

          var web3 = null;

          const permissions = window.ethereum
            ? await window.ethereum.request({ method: "wallet_getPermissions" })
            : [];

          const hasEthAccountsPermissions =
            permissions.map(x => x.parentCapability).includes("eth_accounts");
          ...
          else if ($window.ethereum && hasEthAccountsPermissions) {
          ...
          if (txDefault.wallet == "ledger") {
            if (isElectron) {
              factory.ledgerElectronSetup().then(
                function () {
                  if (resolve) {
                    resolve();
                  }
          ...
          // injected web3 provider (Metamask, mist, etc)
            ...
            // Set accounts
            // Convert to checksummed addresses
            factory.accounts = factory.accounts.map(x => factory.toChecksumAddress(
              x
            ));
            ...
            // }));
            factory.web3 = new Web3(
              new Web3.providers.HttpProvider(txDefault.ethereumNode)
            );
            // Check connection
        ...
        factory.toChecksumAddress = function (item) {
          ...
          if (item instanceof Array) {
            ...
            for (var x = 0; x < item.length; x++) {
              checkSummedItem.push(
                factory.web3.utils.toChecksumAddress(item[x])
              );
            }
          } else if (typeof item == "string") {
            checkSummedItem = factory.web3.utils.toChecksumAddress(item);
          } else if (typeof item == "object") {
            checkSummedItem = {};
            var checkSummedKey;
            for (key in item) {
              checkSummedKey = key.startsWith("0x")
                ? factory.web3.utils.toChecksumAddress(key)
                : key;
              checkSummedItem[checkSummedKey] =
                typeof item[key] == "string" && item[key].startsWith("0x")
                  ? factory.web3.utils.toChecksumAddress(item[key])
                  : item[key];

              if (
                checkSummedItem[checkSummedKey] &&
                checkSummedItem[checkSummedKey].address
              ) {
                checkSummedItem[checkSummedKey].address =
                  factory.web3.utils.toChecksumAddress(
                    checkSummedItem[checkSummedKey].address
                  );
              }
              ...
              if (
                checkSummedItem[checkSummedKey] &&
                checkSummedItem[checkSummedKey].info
              ) {
                // Convert info object to checksummed
                checkSummedItem[checkSummedKey].info =
                  factory.toChecksumAddress(
                    checkSummedItem[checkSummedKey].info
                  );
              }
              if (checkSummedItem[checkSummedKey] && checkSummedKey == "info") {
                // Convert info object to checksummed
                checkSummedItem[checkSummedKey] = factory.toChecksumAddress(
                  checkSummedItem[checkSummedKey]
                );
              }
            }
        ...
        factory.updateAccounts = function (cb) {
          return factory.web3.eth.getAccounts(function (e, accounts) {
            if (e) {
              ...
              if (
                factory.coinbase &&
                factory.accounts.indexOf(factory.coinbase) != -1
              ) {
                // same coinbase
          ...
          var web3Provider = new HookedWalletSubprovider({
            ...
            signTransaction: function (txData, cb) {
              function sendToLedger(chainID) {
                $http
                  .post(
                    "http://localhost:" + ledgerPort + "/sign-transaction",
                    { tx: txData, chain: chainID }
                  )
                  .then(
                    function (tx) {
                      Utils.stopSpinner();
                ...
                TrezorConnect.ethereumGetAddress(
                  ...
                  function (response) {
                    if (response.success) {
                      ...
                      factory.accounts = factory.toChecksumAddress([
                        "0x" + response.address,
                      ]);
                      cb(null, factory.accounts);
                    } else {
          ...
          encryptor
            ...
            .then(function (encryptedV3String) {
              ...
              var generatedAddress =
                factory.toChecksumAddress(_generatedAddress);
        ...
        factory.importLightWalletAccount = function (
          v3,
          password,
          ctrlCallback
        ) {
